...
export default createWorker((env) => ({
  sink: r2BindingSink({ bucket: env.SPOOR_BUCKET }),   // R2 binding, no credentials
  onError: (e) => console.error('[spoor]', e),
}))
```

The Worker runs *around* the origin fetch, so unlike middleware it records the
real `status`, `bytes`, `duration_ms` and `cache_status`. It calls
`passThroughOnException()` before anything else, and never touches `/checkout`.

### Self-hosted, Magento, WooCommerce with shell access

Vector ships the log lines; spoor classifies them. Vector is
[a config, not software](vector/) — it deliberately does no classification,
because a second ruleset written in VRL would drift from the real one.

```bash
vector --config vector/nginx.toml     # tails the access log → local spool
spoor ingest /var/spool/spoor          # classify, verify, truncate → sink
```
...
Answers whether AI crawlers can reach the site *and whether there is anything
for them to read* — a client-rendered SPA returns 200 to GPTBot and an empty
page, which no amount of logging will tell you.
...
| Any site on Cloudflare | `@spoor/worker` | ✅ Shipping |
| **Standard Shopify** | `@spoor/worker` via Cloudflare O2O | ✅ Shipping — ⚠️ **read below** |
| Self-hosted, Magento, WooCommerce with shell | [Vector config](vector/) + `spoor ingest` | ✅ Shipping |
| WooCommerce / WordPress on managed hosting, no shell | [PHP mu-plugin](wordpress/) | ✅ Shipping |
...
| [`@spoor/worker`](packages/worker) | Cloudflare Worker — the only route to standard Shopify |
| [`wordpress/`](wordpress/) | PHP mu-plugin. Shares no code with core, reads the same ruleset |
| [`@spoor/sinks`](packages/sinks) | File, S3/R2 (SigV4, no AWS SDK), R2 binding, webhook, memory |
...
**v0.1, early.** Working end to end across five collection surfaces —
host middleware, Cloudflare Worker, Vector log shipping, Shopify via O2O, and a
WordPress mu-plugin —
plus the classifier, IP-range verification, four sinks, the Parquet roll-up,
the blind-spots report, preflight, and the MCP server. 127 TypeScript tests
and 84 PHP assertions, the latter checked against values generated by the
TypeScript implementation — one ruleset in two languages has to agree, and the
suite proves it rather than assuming it.

All five collection surfaces are built. Next: publishing to npm.
